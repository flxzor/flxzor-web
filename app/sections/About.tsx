...
const About = () => {
  ...
  useGSAP(
    () => {
      ...
      gsap.utils.toArray<HTMLElement>(".about-text-split").forEach((el) => {
  ...
  return (
    <div
      id="about"
      ref={aboutRef}
      className="min-h-screen w-full bg-[#bbc9f1] relative z-10 flex flex-col overflow-hidden"
      style={{
        paddingTop: "100px",
        paddingBottom: "120px",
        paddingLeft: "8vw",
...
<p className="about-text-split text-[#222] text-lg md:text-xl leading-[1.6] font-medium">
  Hey there! I&apos;m <span className="font-bold">Felix</span>, a Software
  ...
  Purwokerto.
        ...
        <div
          className="w-full lg:w-[35%] flex justify-center order-1 lg:order-2 z-20"
          ...
          <Link
            ...
            onClick={(event) => {
              ...
              window.dispatchEvent(
                new CustomEvent("pageTransitionStart", { detail: { href: "/about" } }),
              );
            }}
          >
            <div
              ref={imageRef}
              className="about-image-wrapper relative w-[260px] md:w-[340px] bg-white p-4 pb-16 md:p-5 md:pb-20 shadow-2xl"
            ...
            >
              ...
              <p
                className={`absolute bottom-5 left-5 text-3xl text-[#333] ${caveat.className}`}
              >
                @flxzor
              </p>
            </div>
        ...
        <div className="w-full lg:w-[28%] lg:pb-8 order-3 lg:order-3 text-right lg:text-left flex flex-col items-end lg:items-start">
          <p className="about-text-split text-[#222] text-lg md:text-xl leading-[1.6] font-medium mb-8">
            I’m always curious to learn, build, and improve through every project I take on.
          </p>
        </div>
      </div>
    </div>
